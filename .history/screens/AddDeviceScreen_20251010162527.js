...
export default function AddDeviceScreen({ navigation }) {
  ...
  const handleSave = async () => {
    ...
    try {
      await addDevice({ code: deviceId.trim(), name: deviceName.trim(), location: location.trim() });
      navigation.goBack(); // quay lại DeviceListScreen, useFocusEffect sẽ load lại
    } catch (err) {
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.form}>
        <View style={styles.inputContainer}>
          ...
          <TextInput style={styles.input} placeholder="Mã thiết bị" value={deviceId} onChangeText={setDeviceId} />
        </View>
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput style={styles.input} placeholder="Vị trí lắp đặt" value={location} onChangeText={setLocation} />
        </View>

        <TouchableOpacity style={styles.button} onPress={handleSave}>
          <LinearGradient colors={["#00cc44", "#00aa33"]} style={styles.gradient}>
...
const styles = StyleSheet.create({
  container: { flexGrow: 1, backgroundColor: "#fff0f0", padding: 20 },
  ...
  form: { flex: 1 },
  inputContainer: { flexDirection: "row", alignItems: "center", width: "100%", backgroundColor: "#fff", borderRadius: 15, paddingHorizontal: 15, paddingVertical: 12, marginBottom: 20, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 3 },
  icon: { marginRight: 10 },
  input: { flex: 1, fontSize: 16, color: "#333" },
  button: { width: "100%", marginTop: 10, borderRadius: 15, overflow: "hidden", shadowColor: "#00cc44", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 5, elevation: 5 },
